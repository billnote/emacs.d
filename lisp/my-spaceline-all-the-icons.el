...
(defcustom ati-separator-type 'slant
...
the directions of the separator."
  `(progn
     (spaceline-define-segment
       ...
       (let ((dir (ati-separators--get-direction ,dir))
             (sep (ati-separators--get-type))
             (sf (if (functionp ,start-face) (funcall ,start-face) ,start-face))
             (ef (if (functionp ,end-face) (funcall ,end-face) ,end-face)))

         (when ati-separators-invert-direction
           (setq sf (prog1 ef (setq ef sf))))

         (when (and (eq sep 'slant)          ;; Special case for slant
                    (equal dir "left"))      ;; need to invert the faces for left direction
           (setq sf (prog1 ef (setq ef sf))))

         (unless (or (eq sep 'none)
                     (string= (ati--face-background sf) (ati--face-background ef)))
           (propertize (all-the-icons-alltheicon (format "%s-%s" sep dir) :v-adjust 0.0)
                       'face `(:height 1.3
                                       :family
                                       ,(all-the-icons-alltheicon-family)
                                       :foreground
                                       ,(ati--face-background sf)
                                       :background
                                       ,(ati--face-background ef)))))
       :skip-alternate t :tight t :when (if ,invert (not active) active))))

(setq ati-separators-invert-direction t)
(setq ati-separator-type 'slant)
...
(spaceline-compile
  "ati"
  '(
    ((ati-hud ati-modified ati-window-numbering ati-buffer-size) :face highlight-face :skip-alternate t)
    ;; left-active-3
    ati-left-1-separator
    ((ati-projectile ati-mode-icon ati-buffer-id) :face default-face)
    ati-left-2-separator
    ((ati-process ati-position ati-region-info ati-buffer-position) :face highlight-face :separator " | ")
...
(provide 'my-spaceline-all-the-icons)
