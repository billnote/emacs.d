...
(defun org-dblock-write:block-update-board (params)
  ...
  (let* (todos                  ;; all todos. list of (link . colstr)
    ...
    (insert "#+lATEX: \\tcbset{tabularx={|X|X|X|X|},before upper app={\\rowcolor{DarkCyan} }}\n")
    (insert "#+ATTR_LATEX: :environment tcolorbox :align \"\"\n")
...
(defun org-dblock-write:block-update-summary (params)
  ...
  (let (developers
    ...
    (insert "#+LATEX: \\tcbset{tabularx={X|X|X|X|X|X|X},before upper app={\\rowcolor{DarkCyan} }}\n")
    (insert "#+ATTR_LATEX: :environment tcolorbox :align \"\"\n")
...
(defun block-update-task ()
  "add task table setting"
...
(defun org-dblock-write:block-update-burndown (params)
  ...
  (insert "#+LATEX: \\tcbset{tabularx={X|X|X|X|p{5cm}},before upper app={\\rowcolor{DarkCyan} }}\n")
  (insert "#+ATTR_LATEX: :environment tcolorbox :align \"\"\n")
  (insert "#+tblname: burndown-data\n")
...
(defun scrum-update-all ()
  ...
  (save-excursion
    (let (found)
      ...
      (org-ctrl-c-ctrl-c)
      ;; (goto-char (point-min))
      ;; (setq found (re-search-forward "#\\+NAME: block-update-task" nil t))
      ;; (if (not found)
      ;;     (error "block-update-task not found"))
      ;; (block-update-task)
      (goto-char (point-min))
